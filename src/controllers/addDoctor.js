...
router.post("/",authenticate,upload.single("image"), async (req, res) => {
    try {
      const { username, email, phone,address,gender, DOB,specialist,isAvailable, password } = req.body;
     ...
     return res.status(201).send("image is required");
      ...
      const oldEmail = await docModel.findOne({ email: email });
      if (oldEmail) {
     return res.status(202).send(" Email is already exist");
      }
      const oldPhone = await docModel.findOne({ phone });
      if (oldPhone) {
