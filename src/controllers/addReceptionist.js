...
router.post("/",authenticate,upload.single("image"), async (req, res) => {
    try {
     ...
     return res.status(201).send("image is required");
      ...
      const oldEmail = await receptionistModel.findOne({ email: email });
      if (oldEmail) {
     return res.status(202).send(" Email is already exist");
      }
      const oldPhone = await receptionistModel.findOne({ phone });
      if (oldPhone) {
     ...
     return res.status(202).send(" Phone is already exist");
      ...
      const oldEmail2 = await docModel.findOne({ email: email });
      if (oldEmail2) {
     return res.status(202).send(" Email is already exist");
      }
      const oldPhone2 = await docModel.findOne({ phone });
      if (oldPhone2) {
     return res.status(202).send(" Phone is already exist");
      }
    ...
    const encryptPassword = await bcrypt.hash(password, salt_round);
      const data = await receptionistModel.create({
        username: username,
        email: email,
        phone: phone,
        address:address,
